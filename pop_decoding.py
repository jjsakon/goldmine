...
class PopSpikes(object):
    ...
    def __str__(self):
        ...
        info += (
            "{} time bins/trial".format(
                self.pop_spikes.groupby(["trial"]).size().iloc[0]
            )
            + "\n"
        )
    ...
    def combine_time_bins(self, pop_spikes=None, time_step_size=1000, impute_nans=True):
        ...
        i_game_states = {
            game_state: idx for (idx, game_state) in enumerate(self.game_states)
        }
        _pop_spikes.insert(
            2,
            "iGameState",
            _pop_spikes["gameState"].apply(lambda x: i_game_states[x]).astype(int),
        )
        ...
        _pop_spikes.insert(
            3,
            "time_step",
            _pop_spikes["time_bin"].apply(lambda x: int(x / bins_per_step)),
        )

        # Median impute NaNs.
        if impute_nans:
            _pop_spikes[self.neurons] = _pop_spikes[self.neurons].fillna(
                _pop_spikes[self.neurons].median(axis=0), axis=0
            )

        # For each unit, sum spikes within each time step.
        grp = _pop_spikes.groupby(
            ["trial", "gameState", "iGameState", "time_step"], observed=True
        )
        _pop_spikes = grp[self.neurons].sum().reset_index()

        # Add a column combining game state and time step.
        game_state_times = _pop_spikes.apply(
            lambda x: "{}_{:0>2}".format(int(x["iGameState"]), int(x["time_step"])),
            axis=1,
        )
        i_game_state_times = {
            game_state_time: idx
        ...
        _pop_spikes.insert(
            4, "iGameStateTime", [i_game_state_times[x] for x in game_state_times]
        )
    ...
    def _set_pop_spikes(self):
        ...
        sessions = np.unique(
            [
                op.basename(f).split("-")[0]
                for f in glob(
                    op.join(self.proj_dir, "analysis", "events", "*-Events.pkl")
                )
            ]
        )
        ...
        for subj_sess in sessions:
            if self.place_adj:
                ...
                event_spikes = time_bin_analysis.load_event_spikes(
                    subj_sess, filename=filename, verbose=False
                )
            else:
                event_spikes = time_bin_analysis.load_event_spikes(
                    subj_sess, verbose=False
                )
            neuron_labels = od(
                {
                    neuron: "{}-{}".format(subj_sess, neuron)
        ...
        pop_spikes = pop_spikes.sort_values(
...
def load_pop_spikes(exclude_ctx=True, **kws):
    ...
    if exclude_ctx:
        ...
        keep_idx = ~np.isin(
            [
                spike_preproc.roi_lookup(x.split("-")[0], x.split("-")[1])[1:]
                for x in pop_spikes.neurons
            ],
            ["AI", "O"],
        )
...
def classify_place(
    ...
    def setup_param_search():
        ...
        if search_method == "grid":
            param_grid = {
                "svc__C": np.logspace(-5, 5, vals_per_hyperparam),
    ...
    event_spikes = time_bin_analysis.load_event_spikes(
        subj_sess, proj_dir=proj_dir, verbose=False
    )
    ...
    for game_state in game_states:
        ...
        if verbose:
            print(
                "{} {}: Fitting {} neurons, {:.1f} min".format(
                    subj_sess, game_state, len(neurons), (time() - start_time) / 60
                )
            )
        ...
        for iFold in range(kfold_outer):
            ...
            #
            # Split trials into trainval and test sets.
            X_trainval = _event_spikes.loc[np.isin(_event_spikes["trial"], trainval)][
                neurons
            ].values
            y_trainval = _event_spikes.loc[np.isin(_event_spikes["trial"], trainval)][
                y_col
            ].values
            X_test = _event_spikes.loc[np.isin(_event_spikes["trial"], test)][
                neurons
            ].values
            y_test = _event_spikes.loc[np.isin(_event_spikes["trial"], test)][
                y_col
            ].values
            ...
            # Calculate accuracy.
            accuracy = [
                y_test_pred[iVal] == y_test[iVal] for iVal in range(len(y_test))
            ]
            ...
            shuf_idx = np.concatenate(
                _event_spikes.reset_index()
                .groupby("trial")["index"]
            ...
            X_test_null = _event_spikes_null.loc[
                np.isin(_event_spikes_null["trial"], test)
            ][neurons].values
            y_test_null = _event_spikes_null.loc[
                np.isin(_event_spikes_null["trial"], test)
            ][y_col].values
            X_trainval_null = _event_spikes_null.loc[
                np.isin(_event_spikes_null["trial"], trainval)
            ][neurons].values
            y_trainval_null = _event_spikes_null.loc[
                np.isin(_event_spikes_null["trial"], trainval)
            ][y_col].values
            ...
            # Calculate accuracy.
            accuracy_null = [
                y_test_pred_null[iVal] == y_test_null[iVal]
                for iVal in range(len(y_test_null))
            ]
...
def classify_within_subj(
    ...
    **kws
    ...
    for game_state in game_states:
        # Select a subset of pop_spikes rows and columns.
        _event_spikes = (
            event_spikes.event_spikes.query("(gameState=='{}')".format(game_state))
            .reset_index(drop=True)
            .copy()
        )
        ...
        if y_col == "time_step":
            ...
            _event_spikes["time_step"] = _event_spikes["time_bin"].apply(
                lambda time_bin: int(time_bin / bins_per_step)
            )

            # For each unit, sum spikes within each time step.
            _event_spikes = (
                _event_spikes.groupby(
                    ["gameState", "trial", "time_step"], observed=True
                )[neurons]
                .apply(np.sum)
                .reset_index()
            )
...
def classify_time(
    ...
    basename += "-" + "-".join(
        [
            "{}units_{}".format(len(unit_groups[unit_group]), unit_group)
    ...
    basename += "-{}fold".format(kfold_outer)
    basename += (
        "-{}units_per_subset".format(n_subset_units)
    ...
    basename += "-" + "-".join(
        [
            "{}_{}bins".format(game_state, time_steps_per_game_state[game_state])
            for game_state in game_states
        ]
    )
    ...
    if drop_first2:
        pop_spikes.pop_spikes = pop_spikes.pop_spikes.query(
            "(time_bin!=[0, 1, 2, 3])"
        ).reset_index(drop=True)
    ...
    game_state_durs = events_proc.get_game_state_durs()
    for game_state in game_states:
        n_time_bins = np.unique(
            pop_spikes.pop_spikes.query("(gameState=='{}')".format(game_state))[
                "time_bin"
            ]
        ).size
        ...
        if drop_first2:
            n_time_steps -= int(
                2
                * (
                    time_steps_per_game_state[game_state]
                    / (game_state_durs[game_state] * 1e-3)
                )
            )
        if (n_time_bins % n_time_steps) != 0:
            raise ValueError(
                "n_time_steps does not evenly divide n_time_bins for {} ({} % {} != 0)".format(
                    game_state, n_time_bins, n_time_steps
                )
            )
    ...
    for unit_group in unit_groups:
        ...
        if not np.all(np.isin(_neurons, pop_spikes.neurons)):
            raise ValueError(
                "Some unit names in {} are not in pop_spikes.pop_spikes.columns".format(
                    unit_group
                )
            )
    ...
    for game_state in game_states:
        ...
        for unit_group in unit_groups:
            ...
            # Select a subset of pop_spikes rows and columns.
            _pop_spikes = pop_spikes.pop_spikes.query(
                "(gameState=='{}')".format(game_state)
            ).copy()
            ...
            # For each unit, sum spikes within each time step.
            _pop_spikes = (
                _pop_spikes.groupby(["gameState", "trial", "time_step"], observed=True)[
                    __neurons
                ]
                .apply(np.sum)
                .reset_index()
        ...
        keep_spikes = np.min(
            np.concatenate(
                [
                    [pop_spike_grps[unit_group].iloc[:, 3:].sum().sort_values()]
                    for unit_group in unit_groups
                ],
                axis=0,
            ),
        ...
        for unit_group in unit_groups:
            _keep_spikes = pd.Series(
                data=keep_spikes,
                index=pop_spike_grps[unit_group]
                .iloc[:, 3:]
                .sum()
                .sort_values()
                .index.values,
        ...
        for unit_group in unit_groups:
            ...
            if verbose:
                print(
                    "{}: Fitting {} neurons from {}, {:.1f} min".format(
                        game_state,
                        len(__neurons),
                        unit_group,
                        (time() - start_time) / 60,
            ...
            for iFold in range(kfold_outer):
                ...
                #
                # Split trials into trainval and test sets.
                X_trainval = _pop_spikes.loc[np.isin(_pop_spikes["trial"], trainval)][
                    __neurons
                ].values
                y_trainval = _pop_spikes.loc[np.isin(_pop_spikes["trial"], trainval)][
                    y_col
                ].values
                X_test = _pop_spikes.loc[np.isin(_pop_spikes["trial"], test)][
                    __neurons
                ].values
                y_test = _pop_spikes.loc[np.isin(_pop_spikes["trial"], test)][
                    y_col
                ].values
                ...
                acc_by_time = get_mean_accuracy(
                    accuracy, reshp=(len(test), n_time_steps)
                )
                mean_acc = np.mean(accuracy)
                ...
                # Split trials into trainval and test sets, using the same split as
                # for the observed data.
                X_test_null = _pop_spikes_null.loc[
                    np.isin(_pop_spikes_null["trial"], test)
                ][__neurons].values
                y_test_null = _pop_spikes_null.loc[
                    np.isin(_pop_spikes_null["trial"], test)
                ][y_col].values
                X_trainval_null = _pop_spikes_null.loc[
                    np.isin(_pop_spikes_null["trial"], trainval)
                ][__neurons].values
                y_trainval_null = _pop_spikes_null.loc[
                    np.isin(_pop_spikes_null["trial"], trainval)
                ][y_col].values
                ...
                acc_by_time_null = get_mean_accuracy(
                    accuracy_null, reshp=(len(test), n_time_steps)
                )
                mean_acc_null = np.mean(accuracy_null)
...
def split_trials(trials=None, n_trials=36, n_splits=36):
    """Split trials into trainval and test sets.
    ...
    trainval_test = [
        [trials[idx[0]].tolist(), trials[idx[1]].tolist()] for idx in cv.split(trials)
    ]
