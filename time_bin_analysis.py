...
def run_ols_nav(
    ...
    mult_by=2,
    proj_dir="/home1/dscho/projects/time_cells",
    save_dir="/home1/john/Goldmine/time_cells_goldmine",
    save_output=True,
    overwrite=False,
    verbose=False,
):
    ...
    event_spikes_df = event_spikes.event_spikes
    unique_trials = event_spikes_df['trial'].unique()
    ...
    # 2) Randomly choose half of them
    unique_trials = list(unique_trials)  # convert set to list for np.random.choice
    n_to_remove = len(unique_trials) // 2
    trials_to_remove = np.random.choice(unique_trials, size=n_to_remove, replace=False)

    # 3) Remove those trials from each DataFrame
    trials_to_remove = set(trials_to_remove)  # for faster lookups
    event_spikes.event_spikes = event_spikes_df[~event_spikes_df['trial'].isin(trials_to_remove)]
    events.events            = events.events[~events.events['trial'].isin(trials_to_remove)]
    events.event_times       = events.event_times[~events.event_times['trial'].isin(trials_to_remove)]
    events.positions         = events.positions[~events.positions['trial'].isin(trials_to_remove)]
    events.base_events       = events.base_events[~events.base_events['trial'].isin(trials_to_remove)]
    events.gold_events       = events.gold_events[~events.gold_events['trial'].isin(trials_to_remove)]
    events.gold_spawned      = events.gold_spawned[~events.gold_spawned['trial'].isin(trials_to_remove)]
    events.dig_events        = events.dig_events[~events.dig_events['trial'].isin(trials_to_remove)]
    events.events_behav      = events.events_behav[~events.events_behav['trial'].isin(trials_to_remove)]
    
#     unique_trials = df['trial'].unique()
#     # Randomly choose half of them to remove
#     n_to_remove = len(unique_trials) // 2
#     trials_to_remove = np.random.choice(unique_trials, size=n_to_remove, replace=False)
#     # Filter out rows belonging to those trials
#     df_filtered = df[~df['trial'].isin(trials_to_remove)]
#     # Overwrite the original with the filtered DataFrame
#     event_spikes.event_spikes = df_filtered
