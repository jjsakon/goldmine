...
class Events(object):
    ...
    def __init__(
        ...
        self.events = events_preproc.format_events(
            self.subj_sess,
            overwrite=False,
            save_output=False,
        ...
        self.bad_trials = list(
            np.unique(
                self.events.query("(bad_trials!='')")["trial"].unique().tolist()
                + get_excluded_trials(self.subj_sess)
            )
        )
        self.keep_trials = [
            trial for trial in self.trials if trial not in self.bad_trials
        ]
        self.bad_trial_phases = (
            self.events.query("(bad_trials!='')")[["trial", "gameState"]]
            .drop_duplicates()
            .groupby("trial")["gameState"]
            .apply(lambda x: list(np.unique(x)))
            .reset_index()
            .values.tolist()
        )  # e.g. [[1, ['Delay1']], [4, ['Delay2', 'Retrieval']]]

        # Store a mapping between trial number and whether or not a time
        # penalty was applied.
        self.time_penalty = self.events.query("(key=='trialComplete')").set_index(
            "trial"
        )["time_penalty"]
    ...
    def __str__(self):
        ...
        _s += "{} trials completed, {} good\n\n".format(
            len(self.trials), len(self.keep_trials)
        )

        report_gold = np.all(
            (
                hasattr(self, "gold_spawned"),
    ...
    def log_event_times(self):
        ...
        for iState, game_state in enumerate(game_states):
            dfs.append(
                self._game_state_intervals(
                    self.events, game_state=game_state, cols=["time"]
                )
            )
        ...
        for idx, row in event_times.iterrows():
            if row["gameState"] in game_state_durs:
                ...
                time_bins.append(
                    np.arange(row["time"][0], row["time"][1], step=self.time_bin_dur)
                )
        event_times["time_bins"] = time_bins
        event_times["time_bin_dur"] = event_times["time_bins"].apply(
            lambda x: 0 if len(x) == 0 else x[-1] - x[0]
        )
        event_times["n_time_bins"] = event_times["time_bins"].apply(
            lambda x: np.max((len(x) - 1, 0))
        )
        ...
        event_times = (
            event_times.query("(trial=={})".format(_trials))
    ...
    def log_events_behav(self):
        ...
        for idx, row in self.event_times.iterrows():
            ...
            game_state_dfs = {
                "positions": self.positions.query(
                    "(trial=={}) & (gameState=='{}')".format(trial, game_state)
                ),
                "base_events": self.base_events.query(
                    "(trial=={}) & (gameState=='{}')".format(trial, game_state)
                ),
                "gold_events": self.gold_events.query(
                    "(trial=={}) & (gameState=='{}')".format(trial, game_state)
                ),
                "dig_events": self.dig_events.query(
                    "(trial=={}) & (gameState=='{}')".format(trial, game_state)
                ),
            }
            ...
            for time_bin, (start, stop) in enumerate(
                zip(time_bounds[:-1], time_bounds[1:])
            ):
                if game_state in ["Delay1", "Delay2"]:
                    ...
                    time_step = 1 + int(time_bin / time_step_size[game_state])
                    maze_region, head_direc, is_moving = self._get_position(
                        start, stop, game_state_dfs["positions"]
                    )
                    base_in_view = self._get_base_in_view(
                        start, stop, game_state_dfs["base_events"]
                    )
                    gold_in_view, gold_in_view_ids = self._get_gold_in_view(
                        start, stop, game_state_dfs["gold_events"]
                    )
                    if game_state == "Retrieval":
                        (
                            ...
                            dig_ids,
                            dig_dists,
                        ) = self._get_digs(start, stop, game_state_dfs["dig_events"])
                    else:
        ...
        events_behav = (
            pd.DataFrame(events_behav, columns=cols)
    ...
    def log_positions(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        qry = "(trial=={}) & (gameState=={}) & (key=='playerTransform')".format(
            _trials, game_states
        )
        for idx, df in self.events.query(qry).groupby(["trial", "gameState"]):
            ...
            start_time, stop_time, dur = self._timestamp_intervals(
                df["time"], event_start, event_stop
            )
            ...
            speed = list(
                (
                    1e3
                    * np.array(list(np.linalg.norm(np.diff(pos, axis=0), axis=1)) + [0])
                )
                / np.array(dur)
            )  # m/s
            maze_region = (
                df["value"]
                .apply(
                    lambda x: self.maze.point_to_maze_row(
                        (x["positionX"], x["positionZ"])
                    )
                )
                .tolist()
            )
            ...
            moved_region = [False] + [
                maze_region[i] != maze_region[i + 1]
                for i in range(len(maze_region) - 1)
            ]
            moved_rot = [False] + list(np.diff(rotation) != 0)
            moved_hd = [False] + [
                head_direc[i] != head_direc[i + 1] for i in range(len(head_direc) - 1)
            ]
        ...
        positions = (
            pd.DataFrame(positions).sort_values("start_time").reset_index(drop=True)
        )
    ...
    def log_base_view_events(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        base_events = self.events.query(
            "(trial=={}) & (key=='baseInView')".format(_trials)
        ).copy()
        ...
        base_events = (
            base_events.drop(columns=drop_cols)
            .sort_values("time")
            .reset_index(drop=True)
        )
    ...
    def log_gold_view_events(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        gold_events = self.events.query(
            "(trial=={}) & (key=='goldInView')".format(_trials)
        ).copy()
        ...
        drop_cols = ["key", "value", "scene"]
        gold_events = (
            gold_events.drop(columns=drop_cols)
            .sort_values("time")
            .reset_index(drop=True)
        )
    ...
    def log_gold_spawned(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        gold_spawned = self.events.query(
            "(trial=={}) & (key=='goldLocation')".format(_trials)
        ).copy()

        gold_spawned["gold_id"] = gold_spawned["value"].apply(
            lambda x: x["reportingId"]
        )
        ...
        drop_cols = ["key", "value", "scene"]
        gold_spawned = (
            gold_spawned.drop(columns=drop_cols)
            .sort_values("gold_id")
            .reset_index(drop=True)
        )
    ...
    def log_dig_events(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        dig_events = self.events.query(
            "(trial=={}) & (key=='dig')".format(_trials)
        ).copy()

        dig_events["gold_id"] = dig_events.apply(
            lambda x: self._nearest_gold_id(
                x["trial"],
        ...
        dig_events["dist_from_gold"] = dig_events["value"].apply(
            lambda x: x["distanceFromNearestGold"]
        )

        drop_cols = ["key", "value", "scene"]
        dig_events = (
            dig_events.drop(columns=drop_cols)
            .sort_values("time")
            .reset_index(drop=True)
        )
    ...
    def _game_state_intervals(self, events, game_state, cols=["time"]):
        ...
        output_df = (
            events.query("(gameState=='{}')".format(game_state))
            .groupby("trial")
        ...
        for col in cols:
            output_df[col] = output_df["index"].apply(
                lambda x: [events.loc[x[0], col], events.loc[x[1], col]]
            )

        return output_df

    def _head_direction(self, phi, n=4):
    ...
    def _timestamp_intervals(self, timestamps, start=None, stop=None):
    ...
    def _nearest_gold_id(self, trial, gold_x, gold_z, tol=0.1):
        ...
        dists = _gold_table.apply(
            lambda x: np.linalg.norm((x["gold_x"] - gold_x, x["gold_z"] - gold_z)),
            axis=1,
        ).tolist()
    ...
    def _get_position(self, start, stop, positions):
        ...
        pos_idx = np.arange(
            positions.loc[(positions["start_time"] > start)].index[0] - 1,
        ...
        maze_region = (
            _positions.groupby("maze_region")["dur"]
            .apply(np.sum)
            .sort_values()
            .index[-1]
        )
        head_direc = (
            _positions.groupby("head_direc")["dur"]
            .apply(np.sum)
            .sort_values()
            .index[-1]
        )
        is_moving = np.any(_positions["moved_pos"] | _positions["moved_rot"]).astype(
            int
        )

        return maze_region, head_direc, is_moving

    def _get_base_in_view(self, start, stop, base_events):
        ...
        base_in_view : 0 or 1
            ...
            time window, else 0.
        """
        _base_events = base_events.loc[
            (base_events["time"] >= start) & (base_events["time"] <= stop)
        ]
    ...
    def _get_gold_in_view(self, start, stop, gold_events):
        ...
        """
        _gold_events = gold_events.loc[
            (gold_events["time"] >= start) & (gold_events["time"] <= stop)
        ]
        ...
        if gold_in_view:
            gold_in_view_ids = list(
                np.unique(_gold_events.loc[_gold_events["in_view"] == 1, "gold_id"])
            )
        else:
            gold_in_view_ids = np.nan
    ...
    def _get_digs(self, start, stop, dig_events):
        ...
        """
        _dig_events = dig_events.loc[
            (dig_events["time"] >= start) & (dig_events["time"] <= stop)
        ]
...
class Maze(object):
    ...
    def __init__(self, maze_name, proj_dir="/home1/dscho/projects/time_cells"):
    ...
    def __str__(self):
        ...
        s += "{} floor tiles in the base ({} m^2)\n".format(
            len(base_tiles), base_tiles["area"].sum()
        )
        s += "{} floor tiles in the mine ({} m^2)\n".format(
            len(mine_tiles), mine_tiles["area"].sum()
        )
        s += "{} inner wall tiles ({} m^2)\n".format(
            len(iwall_tiles), iwall_tiles["area"].sum()
        )
        s += "{} outer wall tiles ({} m^2)".format(
            len(owall_tiles), owall_tiles["area"].sum()
        )

        return s

    def point_to_maze_row(self, point, col="region_precise"):
        ...
        contains_point = (
            self.maze["coords"].apply(lambda x: _point_in_coord(point, x)).tolist()
        )
...
def save_events(events, overwrite=False, verbose=True):
...
def load_events(
    ...
    **kwargs
    ...
    if filename is None:
        filename = op.join(
            proj_dir, "analysis", "events", "{}-Events.pkl".format(subj_sess)
        )
    ...
    if op.exists(filename) and not overwrite:
        ...
        events = Events(
            subj_sess, proj_dir=proj_dir, filename=filename, verbose=verbose, **kwargs
        )
    ...
    if hasattr(events, "events_behav"):
        ...
        game_state_cat = pd.CategoricalDtype(game_states, ordered=True)
        events.events_behav["gameState"] = events.events_behav["gameState"].astype(
            game_state_cat
        )
