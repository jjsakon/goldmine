...
def load_spikes(subj_sess, neuron, proj_dir='/data7/goldmine/'):
...
def add_null_to_spikes(
    ...
    output_f = op.join(
        proj_dir, "analysis", "spikes", "{}-spikes.pkl".format(subj_sess)
    )
    if spikes is None:
    ...
    spikes[output_col] = (
        spikes["spike_times"]
        .apply(lambda x: create_null_spike_times(x, event_times, n_perms))
        .tolist()
    )
    ...
    if save_output:
        ...
        if split_files:
            output_f = op.join(
                proj_dir, "analysis", "spikes", "{}-CSC{}-unit{}-spikes.pkl"
            )
            for idx, row in spikes.iterrows():
                dio.save_pickle(
                    row,
                    output_f.format(row["subj_sess"], row["chan"], row["unit"]),
                    verbose,
                )
        else:
...
def apply_shift_spike_times(event_window, spike_times):
    ...
    spike_times_shifted_ = shift_spike_inds(
        spike_times_, shift_by, floor=start, ceiling=stop
    )

    return spike_times_shifted_


def apply_spike_times_to_fr(event_window, spike_times, sigma=500):
    ...
    if len(shp) == 1:
        ...
        spike_times_ = np.array(
            [
                spike_times[
                    x, (spike_times[x, :] >= start) & (spike_times[x, :] < stop)
                ]
...
def create_null_spike_times(spike_times, event_times, n_perms=1000):
    ...
    """
    spike_times_shifted = np.array(
        [
            np.concatenate(
                event_times["time"].apply(
                    lambda x: apply_shift_spike_times(x, spike_times)
                )
            )
...
def event_fr(
    ...
    def mean_fr(time_range, spike_times, bins=None):
        ...
        if (bins is None) & (len(spike_times.shape) == 1):
            return np.mean(apply_spike_times_to_fr(time_range, spike_times, sigma))
        ...
        # Returns a vector whose length is the number of permutations.
            ...
            return np.mean(
                apply_spike_times_to_fr(time_range, spike_times, sigma), axis=-1
            )
            ...
            return np.array(
                [
                    ...
                    for arr in np.array_split(
                        apply_spike_times_to_fr(time_range, spike_times, sigma), bins
                    )
                ]
            )
        ...
        # Returns a perm x time_bin array.
            ...
            return np.swapaxes(
                [
                    np.mean(arr, axis=-1)
                    for arr in np.array_split(
                        apply_spike_times_to_fr(time_range, spike_times, sigma),
                        bins,
                        axis=-1,
                    )
                ],
    ...
    output_f = op.join(
        ...
        "analysis",
        "fr_by_time_bin",
        "{}-CSC{}-unit{}-event_fr.pkl".format(
            spikes["subj_sess"], spikes["chan"], spikes["unit"]
        ),
    )
    ...
    if trial_times is not None:
        ...
        if include_trial_null:
            ...
            trial_times["fr_null"] = trial_times["time"].apply(
                lambda x: mean_fr(x, spike_times_trial_null)
            )

    if event_times is not None:
        event_times["fr"] = event_times.apply(
            lambda x: mean_fr(x["time"], spike_times, x["n_time_bins"]), axis=1
        )
        if include_event_null:
            spike_times_event_null = spikes["spike_times_null_trial_phase"]
            event_times["fr_null"] = event_times.apply(
                lambda x: mean_fr(x["time"], spike_times_event_null, x["n_time_bins"]),
                axis=1,
            )
...
def format_spikes(
    ...
    lfp_timestamps, sr = spike_sorting.load_lfp_timestamps(
        subj_sess=subj_sess, conv_factor=conv_factor, proj_dir=proj_dir, verbose=verbose
    )
    sess_stop_time = np.int(lfp_timestamps[-1])
    session_dur = lfp_timestamps[-1] - lfp_timestamps[0]

    if add_montage_info:
        elec_montage_f = op.join(
            proj_dir, "data", subj, sess, "micro_lfps", "anatleads.txt"
        )
        assert op.exists(elec_montage_f)

    # Get all spike files for the session.
    spike_files = np.array(
        glob(
            op.join(
                proj_dir, "data", subj, sess, "spikes", spikes_dirname, "times_CSC*.mat"
            )
        )
    )
    ...
    # Reorder spike files.
    chans = np.array(
        [np.int(op.basename(f).split("CSC")[1].split(".")[0]) for f in spike_files]
    )
    ...
    spikes["spike_times"] = spikes["spike_times"].apply(
        lambda x: np.rint(x).astype(np.int)
    )
    ...
    if add_montage_info:
        ...
        mont = od(
            [
                (x[0], np.arange(int(x[1].split("-")[0]), int(x[1].split("-")[1]) + 1))
                for x in mont
            ]
        )
    ...
    if verbose:
        print(
            spikes.groupby("hemroi").agg(
                {"unit": len, "chan": lambda x: len(np.unique(x)), "fr": np.median}
            )
        )
...
def roi_lookup(subj_sess, chan, proj_dir='/data7/goldmine/'):
...
def get_montage(subj_sess, proj_dir='/data7/goldmine/'):
...
def shift_spike_inds(spike_inds, step, floor=0, ceiling=np.inf):
    ...
    if step < 0:
        ...
        spike_inds_shifted[spike_inds_shifted < 0] = (
            spike_inds_shifted[spike_inds_shifted < 0] + ceiling
        )
    else:
        roll_by = len(spike_inds_shifted[spike_inds_shifted >= ceiling])
        spike_inds_shifted[spike_inds_shifted >= ceiling] = (
            spike_inds_shifted[spike_inds_shifted >= ceiling] - ceiling
        )
...
def spike_times_to_fr(spike_times, stop_time, sigma=500):
    ...
    if len(shp) == 1:
        ...
        spike_train = np.array(
            [spike_times_to_train(spike_times[x, :], stop_time) for x in range(shp[0])]
        )
    if sigma > 0:
        fr = (
            gaussian_filter1d(spike_train, sigma, mode="reflect", axis=-1) * 1e3
        )  # convert ms to s
        return fr
    else:
        return spike_train
...
def spike_times_to_train(spike_times, stop_time):
